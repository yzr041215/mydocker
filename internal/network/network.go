...
import (
	...
	"engine/internal/constant"
	container "engine/internal/containers"
...
func Connect(networkName string, info *container.ContainerInfo) (net.IP, error) {
	...
	// 创建网络端点
	ep := &Endpoint{
		ID:          fmt.Sprintf("%s-%s", info.ContainerID, networkName),
		IPAddress:   ip,
		Network:     network,
...
func Disconnect(networkName string, info *container.ContainerInfo) error {
	...
	drivers[network.Driver].Disconnect(fmt.Sprintf("%s-%s", info.ContainerID, networkName))
...
func enterContainerNetNS(enLink *netlink.Link, info *container.ContainerInfo) func() {
	...
	f, err := os.OpenFile(fmt.Sprintf("/proc/%d/ns/net", info.Pid), os.O_RDONLY, 0)
	if err != nil {
